...
def age_of_domain(domain):
    ...
    expiration_date = domain.expiration_date
    ageof domain = abs((expiration_date - creation_date).days)
...
def web_traffic(url):
    try:
        rank = bs4.BeautifulSoup(urllib.urlopen("http://data.alexa.com/data?cli=10&dat=s&url=" + url).read(), "xml").find(
            "REACH")['RANK']
    except TypeError:
        return -1
    rank = int(rank)
...
def statistical_report(url, hostname):
    ...
    ip_match = re.search(
        ...
        '216\.38\.62\.18|104\.130\.124\.96|47\.89\.58\.141|78\.46\.211\.158|54\.86\.225\.156|54\.82\.156\.19|37\.157\.192\.102|204\.11\.56\.48',
        ip_address)
    if url_match:
        return -1
    elif ip_match:
